    ...
    public class SecondWindowViewModel
    {
        ...
        public static string SzereloID(IDbConnection connection, string szerelonev)
        {
            ...
            command.CommandText = "SELECT DISTINCT szerelok.szerelo_id " +
                "FROM szerelok " +
                "INNER JOIN karbantartasok on szerelok.szerelo_id = karbantartasok.szerelo_id " +
        ...
        public static string MegrendeloID(IDbConnection connection, string megrendelonev)
        {
            ...
            command.CommandText = "SELECT DISTINCT megrendelok.megrendelo_id " +
                "FROM megrendelok " +
                "INNER JOIN karbantartasok on megrendelok.megrendelo_id = karbantartasok.megrendelo_id " +
            ...
            using var reader = command.ExecuteReader();
            reader.Read();
            string result = reader["megrendelo_id"].ToString(); //Javítani: 10-es ID megrendelőt nem lehet felvinni
            reader.Close();
            return result;
        }
